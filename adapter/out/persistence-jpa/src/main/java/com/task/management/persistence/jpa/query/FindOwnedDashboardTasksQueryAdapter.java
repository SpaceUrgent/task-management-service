...
public class FindOwnedDashboardTasksQueryAdapter implements FindPageQuery<TaskEntity> {
    private final FindOwnedDashboardTasksQuery query;

    public FindOwnedDashboardTasksQueryAdapter(FindOwnedDashboardTasksQuery query) {
        this.query = query;
    }
    ...
    private Predicate[] criteriaPredicateArray(CriteriaBuilder criteriaBuilder,
        ...
        final var joinOwner = root.join("owner", JoinType.INNER);
        predicates.add(criteriaBuilder.equal(joinOwner.get("id"), query.getOwner().value()));
