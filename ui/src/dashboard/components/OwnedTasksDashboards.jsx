...
export default function OwnedTasksDashboards() {
        ...
        const fetchInitialData = async () => {
            try {
                const [summaryData, overdueData, tasksData] = await Promise.all([
                    dashboardClient.getOwnedTaskSummary(),
                    dashboardClient.getOverdueOwnedTasks({ page: 0, size: 5 }),
                    dashboardClient.getOwnedTasks({ page: 0, size: 10 })
                ]);
    ...
    const handleLoadMoreOverdue = async (nextPage) => {
        try {
            const response = await DashboardClient.getOverdueOwnedTasks({
                page: nextPage,
                size: 5
            });
    ...
    const handleLoadMoreTasks = async (nextPage) => {
        try {
            const response = await DashboardClient.getOwnedTasks({
                page: nextPage,
                size: 10
            });
